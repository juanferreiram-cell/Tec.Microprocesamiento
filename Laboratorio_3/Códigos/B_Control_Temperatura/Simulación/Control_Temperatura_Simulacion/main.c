...
static uint16_t uart_leer_entero(void){
	uint16_t valor = 0;
...
// Constantes para recalcular al cambiar el rango del punto medio
...
// Funciones para mostrar el rango actual
...
static void comando_reconfigurar_rango(void){
	...
	uint16_t min_medio = uart_leer_entero();

	uart_tx_cadena("Ingrese maximo del nuevo rango: ");
	uint16_t max_medio = uart_leer_entero();
...
static void procesar_uart(void){
	while (uart_rx_disponible()){
		...
		if (c=='E'){
			...
			uart_tx_cadena("Comandos: E=Calefactor ENCENDIDO, A=Calefactor APAGADO, R=Rango medio\r\n");
		}
	}
}
